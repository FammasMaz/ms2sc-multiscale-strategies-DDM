function [u, K, Kord, F, Ford, h] = FEM(nbSub, nbLocalElems, plt)
  % Solver for FEM Problem Structured and Unstructured Problem using direct method
  %   [u, K, Kord, F, Ford, h] = FEM(nbSub, nbLocalElems, plt)
  % Output :  u = total displacement vector
  %           K = original assembled stiffness matrix
  %           Kord = ordered assembled stiffness matrix
  %           F = original assembled force vector
  %           Ford = ordered assembled force vector
  %           h = mesh size
