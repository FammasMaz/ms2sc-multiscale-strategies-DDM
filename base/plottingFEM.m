function plottin(uxy)
    ...
    dXY = [truss.nodes 0.2*ones(size(uxy,1),1)] + [uxy 0.2*ones(size(uxy,1),1)]; % Coordinates of the deformed matrix
    ...
    gplot(adjac, dXY, "b");
    ylim([-0.4 0.8]);
    title('Displacement without Reordering');
