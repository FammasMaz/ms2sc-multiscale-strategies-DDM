function [Sps, bps, Kii, Kib, fi, ke, Fd] = fem_k_dual(truss, sol, Sp_gen)
  %% Function for generating domain wise Sps, bps, Kii, Kib, fi, ke, Fd
  % Inputs:  truss - Structure containing all the data
  %          sol - 1 solving the problem, 0 not solving the problem
  %          Sp_gen - 1 generating the Sps, 0 for generating the pseudo inverse
  % Outputs: Sps - Primal Schur of the domain
  %          bps - Force vector of the domain
  %          Kii - Stiffness matrix of the internal nodes
  %          Kib - Stiffness matrix of the internal and boundary nodes correlated
  %          fi - Force vector of the internal nodes
  %          ke - Stiffness matrix of the element
  %          Fd - Force vector of the domain
